...
/*Inizializzo*/
	...
	list_mux=mutex->mux;
printf("MUTEX CREATO %d, %d\nInizializzato%d %d\n",ACTIVE,mutex->mux->state,INIT,initialized);	
    if(initialized==INIT)
        return SETERR(EBUSY);
	return SETERR(OK);
}
...
{
	mutexWait* nuovo=malloc(sizeof(mutexWait));
	if(!nuovo)
	return SETERR(ENOMEM);
	nuovo->next=NULL;
	nuovo->own=ESECUTION_TID;
	if(!mutex->mux->list_head)
	mutex->mux->list_head=mutex->mux->list_tail=nuovo;
	else{
	mutex->mux->list_tail->next=nuovo;
	mutex->mux->list_tail=nuovo;
	}
	pth_sleep(ESECUTION_TID,LOCKSLEEP);
	printf("ADDIIIIIIRITTURA %d %d TID%d\n",thread_exec->state,ZOMBIE,thread_exec->tid);
